...
$(async function () {
  ...
  commentForm.on('submit', async (e) => {
    ...
    await $.ajax({
      type: "POST",
      url: `https://conscientiz-ai-server.onrender.com/comments/${postId}`,
...
async function loadPostContent() {
  await $.ajax({
    type: "GET",
    url: `https://conscientiz-ai-server.onrender.com/posts/${postId}`,
    headers: { "Accept": "application/json" },
  })
    .done((data, textStatus, jqXHR) => {
...
function setupUpdateRating() {
  ...
  setTimeout(() => {
    if (ratingState.dirty) {
      ...
      $.ajax({
        type: "POST",
        url: `https://conscientiz-ai-server.onrender.com/posts/${postId}/rating`,
        headers: { "Accept": "application/json" },
        contentType: 'application/json',
